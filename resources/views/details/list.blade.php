<div class="card mb-3 ">
    <div class="row h-200">
        <div class="col-lg-6">
            <div class="img-block">
                
                <img class="img-sell card-img-top" src="https://www.planmodernhome.com/images/md06/md06.png" alt="listings" >

            </div>   
                 
        </div>
        <div class="text-list  text-center">
            <label class="label-700 pt-1 ml-2 font-label"> $14,000 / mo </label>
        </div>
        <div class="block-list text-center">
            <label class="label-700 pt-1 pl-1 font-label"> $FEATURED </label>
        </div>

        <div class="col-lg-5">
            <div class="card-body">
                <h6 class="card-title"><a href="">Equestrian Family Home</a></h6>
                <p class="list-text">San Diego City, CA, USA</p>
                <div class="d-flex align-items-center">
                    <a href="#" class="mr-10"><i class="fa-solid fa-bed"></i> 5 bed</a>
                    <a href="#" class="mr-10"><i class="fa-solid fa-bath"></i> 4 bath</a>
                    <a href="#" class="mr-10"> <i class="fa-brands fa-uncharted"></i> 900 sqft</a>
                </div>
            </div>
            <div class="card-footer bg-transparent">
                <div class="d-flex bd-highlight">
                    <div class="flex-grow-1 bd-highlight"> <label class="label-700">For Rent</label>
                    </div>
                    <div class="bd-highlight">
                        <i class="fa-regular fa-share-from-square mr-10"aria-hidden="true"></i>
                        <i class="fa fa-heart-o mr-10" aria-hidden="true"></i>
                        <i class="fa-regular fa-square-plus mr-10" aria-hidden="true"></i>
                    </div>
                </div>
            </div>
        </div>
        
    </div>
</div>


<div class="card mb-3 ">
    <div class="row h-200">
        <div class="col-lg-6">
            <div class="img-block">
                
                <img class="img-sell card-img-top" src="https://www.black-beam.com/plans/BB-H2-60002.15_3d1.jpg" alt="listings" >

            </div>   
                 
        </div>
        <div class="text-list  text-center">
            <label class="label-700 pt-1 ml-2 font-label"> $14,000 / mo </label>
        </div>
        <div class="col-lg-5">
            <div class="card-body">
                <h6 class="card-title"><a href="">Equestrian Family Home</a></h6>
                <p class="list-text">San Diego City, CA, USA</p>
                <div class="d-flex align-items-center">
                    <a href="#" class="mr-10"><i class="fa-solid fa-bed"></i> 5 bed</a>
                    <a href="#" class="mr-10"><i class="fa-solid fa-bath"></i> 4 bath</a>
                    <a href="#" class="mr-10"> <i class="fa-brands fa-uncharted"></i> 900 sqft</a>
                </div>
            </div>
            <div class="card-footer bg-transparent">
                <div class="d-flex bd-highlight">
                    <div class="flex-grow-1 bd-highlight"> <label class="label-700">For Rent</label>
                    </div>
                    <div class="bd-highlight">
                        <i class="fa-regular fa-share-from-square mr-10"aria-hidden="true"></i>
                        <i class="fa fa-heart-o mr-10" aria-hidden="true"></i>
                        <i class="fa-regular fa-square-plus mr-10" aria-hidden="true"></i>
                    </div>
                </div>
            </div>
        </div>
        
    </div>
</div>

